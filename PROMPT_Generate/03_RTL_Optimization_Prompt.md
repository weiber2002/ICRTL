...
Six sequential turns, same chaining rule as the generation template: each stage's output feeds the next.
...
>
> Six turns: (1) understand current implementation, (2) algorithm-level improvement, (3) micro-architecture-level improvement, (4) interface delta, (5) — *(interface delta is folded into stage 5 numbering below)* — code generation. Don't skip to code before the earlier stages are done.
