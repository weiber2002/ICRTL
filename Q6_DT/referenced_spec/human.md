...
**Top module name:** `TOP`  (Verilog: `module TOP (...);`)

**Clocking & rst**

* `clk`: rising-edge synchronous.
* `rst`: **active-high, Synchronous**. While `rst=1`, clear state and do not produce valid writes/`done`. The testbench holds `rst=1` for \~2 cycles then deasserts to `0`.&#x20;
...
1. **rst** (`rst=1` for \~2 cycles): clear all internal state; no reads/writes; `done=0`.
2. **ROM read / compute:** after `rst` deasserts (`rst=0`), drive `sti_rd/sti_addr` to fetch binary data packs from `sti_ROM` and perform the DT algorithm. You **may** use `res_RAM` as temporary storage via `res_rd/res_wr` during forward/backward passes.
