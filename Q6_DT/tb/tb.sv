...
`define TB1
`ifdef TB1
	`define PAT        "./pattern/Geometry_sti.dat"    
	`define FWEXP        "./pattern/Geometry_fwexp.dat"  
	`define BCEXP        "./pattern/Geometry_bcexp.dat"      
`elsif TB2
	`define PAT        "./pattern/ICC17_sti.dat"    
	`define FWEXP        "./pattern/ICC17_fwexp.dat"
	`define BCEXP        "./pattern/ICC17_bcexp.dat"
`endif
...
initial begin
      ...
      if( bc_err == 0 ) begin
		...
		err_percent = ((bc_err)/N_EXP)*100.0;
		total_error = bc_err;
		$display(bc_err);
