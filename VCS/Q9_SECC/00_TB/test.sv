`timescale 1ns/10ps
`define SDFFILE    "../02_SYN/Netlist/top_syn.sdf"    // Modify your sdf file name here
`define cycle 10.0
...
TOP TOP( .clk(clk), .rst(rst), .en(en), .central(central), .radius(radius), .mode(mode), .busy(busy), .valid(valid), .candidate(candidate) );
