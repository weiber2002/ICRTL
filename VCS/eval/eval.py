...
power_total_str = grep(r'Total Power\s*=\s*([\d\.eE+-]+)', power_txt)
# report_power 的 Total Power 單位是 W,乘以 1000 換算成 mW
power_total = float(power_total_str) * 1000 if power_total_str else ''

# WNS (data arrival time of the worst max-delay path)
wns_path = os.path.join(ROOT, '04_POWER', 'top.wns')
wns = ''
if os.path.exists(wns_path):
    wns_txt = open(wns_path).read().strip()
    try:
        wns = float(wns_txt)
    except ValueError:
        wns = ''
...
try:
    ppa_total = float(area_total) * float(evaluation_time_ns) * power_total
except Exception:
    ppa_total = ''
