...
| `reset`    | I   |     1 | **Asynchronous, active-high** reset.                                                                                                                        |
| `ready`    | I   |     1 | Testfixture asserts when inputs are prepared; `CONV` may start only after seeing `ready=1`.                                                                 |
...
1. **Start/End protocol**
   ...
   * After `reset` deasserts, testfixture sets `ready=1` when inputs (image & constants) are ready. On detecting `ready=1`, assert `busy=1` (start). When all mandated writes (L0 then L1) complete, deassert `busy` (end). **Exactly one** rising and **one** falling edge of `busy` per image. &#x20;
