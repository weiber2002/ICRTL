module  TOP(
	...
	// 	IDLE, INPUT, L0_MEM, DELAY_CLK, READ_L0_MEM, L1_MEM, DELAY_CLK_2, FINISH
	// } state_t;
	...
	always@( posedge clk or posedge rst ) begin
		if( rst ) iaddr <= 0;
			...
			case( counter_addr )
				0: iaddr <= {y - 1'd1,x - 1'd1};
				1: iaddr <= {y - 1'd1,x};
				2: iaddr <= {y - 1'd1,x + 1'd1};
				3: iaddr <= {y,x - 1'd1};
				4: iaddr <= {y,x};
				5: iaddr <= {y,x + 1'd1};
				6: iaddr <= {y + 1'd1,x - 1'd1};
				7: iaddr <= {y + 1'd1,x};
				8: iaddr <= {y + 1'd1,x + 1'd1};
				endcase
		end
	...
	always@(posedge clk or posedge rst) begin
		if( rst ) idata_tmp <= 0;
			...
			case( counter_data )
				0: idata_tmp <= ( x == 0 || y == 0 )? 0 : idata;
				1: idata_tmp <= ( y == 0 )? 0: idata;
				2: idata_tmp <= ( x == 63 || y == 0 )? 0: idata;	
				3: idata_tmp <= ( x == 0 )? 0: idata;
				4: idata_tmp <= idata;
				5: idata_tmp <= ( x == 63 )? 0: idata;
				6: idata_tmp <= ( x == 0 || y == 63 )? 0: idata;
				7: idata_tmp <= ( y == 63 )? 0: idata;
				8: idata_tmp <= ( x == 63 || y == 63 )? 0: idata;
			endcase
		end 
	end
	...
	always@(posedge clk or posedge rst) begin
		if( rst ) caddr_rd <= 0;
			...
			case( counter_layer1 )
				0: caddr_rd <= {L1_y,L1_x};
				1: caddr_rd <= {L1_y,L1_x+1'd1};
				2: caddr_rd <= {L1_y+1'd1,L1_x};
				3: caddr_rd <= {L1_y+1'd1,L1_x+1'd1};
			endcase 
		end
	end
