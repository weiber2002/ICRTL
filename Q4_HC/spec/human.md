...
* Synchronous design on **posedge `clk`**. Reset is **asynchronous active-high**.
...
| `reset`      |   I |      1 | **Asynchronous, active-high** system reset. While `reset=1`: internal state cleared; no valid outputs asserted. |
...
### 5.1 Reset and input (T1, T2, T3)

* **T1**: `reset=1` is asserted for at least two cycles. During `reset=1`, clear all internal state and do **not** assert `CNT_valid` or `code_valid`. After `reset` deasserts (1→0), the core is ready.
